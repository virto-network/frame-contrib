...
/// A track is updated by the origin it belongs to (e.g. a community updates its own track).
pub struct EnsureTrackOrigin;
impl EnsureOriginWithArg<RuntimeOrigin, CommunityId> for EnsureTrackOrigin {
    type Success = ();

    fn try_origin(o: RuntimeOrigin, id: &CommunityId) -> Result<Self::Success, RuntimeOrigin> {
        use pallet_referenda::TracksInfo;
        match CommunityTracks::track_for(&o.caller) {
            Ok(track_id) if &track_id == id => Ok(()),
            _ => Err(o),
        }
    }

    #[cfg(feature = "runtime-benchmarks")]
    fn try_successful_origin(id: &CommunityId) -> Result<RuntimeOrigin, ()> {
        fc_pallet_referenda_tracks::OriginToTrackId::<Runtime>::iter()
            .find(|(_, track_id)| track_id == id)
            .map(|(origin, _)| origin.into())
            .ok_or(())
    }
}
...
impl fc_pallet_referenda_tracks::Config for Runtime {
    ...
    type UpdateOrigin = EnsureTrackOrigin;
